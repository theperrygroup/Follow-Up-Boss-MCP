...
| `FOLLOWUPBOSS_DEFAULT_TIMEZONE` | No | None | IANA timezone name (such as `America/Denver`) used to interpret naive appointment and task datetimes. Follow Up Boss stores times in UTC and does not honor a timezone offset suffix on the wire, so the server converts datetimes to UTC before sending: naive values are interpreted in this zone, and aware values are converted from their offset. When unset, naive values are sent unchanged and Follow Up Boss treats them as UTC. The legacy alias `FOLLOW_UP_BOSS_DEFAULT_TIMEZONE` is also accepted. |
