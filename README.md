...
| `FOLLOWUPBOSS_DEFAULT_TIMEZONE` | No | None | IANA timezone name (such as `America/Denver`) applied to appointment and task datetimes that arrive without a UTC offset. Follow Up Boss treats offset-less datetimes as UTC, so set this to the user's local zone to avoid shifted appointments. Datetimes that already include an offset are never changed. The legacy alias `FOLLOW_UP_BOSS_DEFAULT_TIMEZONE` is also accepted. |
