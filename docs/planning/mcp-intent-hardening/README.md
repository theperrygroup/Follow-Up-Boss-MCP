...
| Runtime feature or implementation truth | Current working tree includes intent-scoped helpers for latest assigned lead, authenticated-user task due buckets, and the first read-only battle-test evaluator. Broader intent taxonomy, real transcript capture, and mutation validation remain planned. |
| Highest-risk remaining surface | Vague chatbot prompts have not yet been exercised through a real MCP client transcript capture run with persisted evidence. |
