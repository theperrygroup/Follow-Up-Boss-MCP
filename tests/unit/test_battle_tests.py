...
@pytest.mark.asyncio
async def test_zero_communication_grounding_rejects_list_lookup_or_activity_detour() -> None:
    scenario = scenario_by_id("BT-READ-018")
    services = _services(
        people=_people_page(
            PersonRecord.model_validate(
...
@pytest.mark.asyncio
async def test_named_smart_list_grounding_rejects_unowned_my_followup_route() -> None:
    scenario = scenario_by_id("BT-SMARTLIST-002")
    services = _services(
        smart_lists=_smart_lists_page(
            SmartListRecord.model_validate({"id": 77, "name": "Eligible For Transfer"})
